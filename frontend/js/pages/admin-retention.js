    ...
    <div style="max-width:1100px;margin:32px auto;padding:0 20px">
      ...
      <div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:24px">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;margin-bottom:16px">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button id="btn-preview" onclick="sendRetentionPreview(this)"
              style="background:#14532d;border:1px solid #4ade8055;border-radius:6px;padding:7px 14px;color:#4ade80;cursor:pointer;font-size:0.82rem;white-space:nowrap">
              📧 ${isEN ? 'Sample to me' : isPT ? 'Amostra p/ mim' : 'Muestra a mí'}
            </button>
            <button id="btn-send-all" onclick="sendRetentionAll(this)"
              style="background:#7c3aed22;border:1px solid #a78bfa55;border-radius:6px;padding:7px 14px;color:#a78bfa;cursor:pointer;font-size:0.82rem;font-weight:600;white-space:nowrap">
              🚀 ${isEN ? 'Send to all' : isPT ? 'Enviar p/ todos' : 'Enviar a todos'}
            </button>
            <button onclick="loadRetentionData()"
              style="background:none;border:1px solid var(--border);border-radius:6px;padding:7px 12px;color:var(--text2);cursor:pointer;font-size:0.82rem;white-space:nowrap">
        ...
        <input type="text" id="retention-search"
          ...
          style="width:100%;box-sizing:border-box;background:var(--input);border:1px solid var(--border);border-radius:8px;padding:9px 14px;color:var(--text1);font-size:0.9rem;margin-bottom:16px" />
...
async function loadRetentionData() {
  ...
  try {
    ...
    const steps = [
      ...
      { key: 'analisis_mano', es: 'Análisis Mano',    pt: 'Análise de Mão',   en: 'Hand Analysis' },
    ];
    ...
    funnel.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:12px">
        ${steps.map(s => {
          ...
          return `
            <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center">
              <div style="font-size:1.6rem;font-weight:700;color:#a78bfa">${count}</div>
        ...
        <div style="background:var(--card);border:1px solid #4ade8055;border-radius:10px;padding:14px;text-align:center">
          <div style="font-size:1.6rem;font-weight:700;color:#4ade80">${data.completados}</div>
...
function renderRetentionTable(users) {
  ...
  const chk = (val) => val
    ...
    : '<span style="color:#ef444466;font-size:0.9rem">✗</span>';
  ...
  const rows = users.map(u => {
    ...
    const opened = u.retention_email_opened_at
      ? `<span style="color:#4ade80;font-size:0.78rem;white-space:nowrap">👁 ${new Date(u.retention_email_opened_at).toLocaleDateString()}</span>`
      : `<span style="color:#334155;font-size:0.78rem">—</span>`;
    return `
      <tr style="border-top:1px solid var(--border);transition:background 0.15s" onmouseover="this.style.background='#ffffff08'" onmouseout="this.style.background=''">
        <td style="padding:10px 12px;color:var(--text1);font-size:0.88rem;font-weight:500;white-space:nowrap">${u.nombre}</td>
        <td style="padding:10px 12px;color:var(--text3);font-size:0.8rem">${u.email}</td>
        <td style="padding:10px 8px;text-align:center">${chk(u.cycle.test_mental)}</td>
        <td style="padding:10px 8px;text-align:center">${chk(u.cycle.test_tecnico)}</td>
        <td style="padding:10px 8px;text-align:center">${chk(u.cycle.perfil_ia)}</td>
        <td style="padding:10px 8px;text-align:center">${chk(u.cycle.analisis_mano)}</td>
        <td style="padding:10px 8px;color:var(--text3);font-size:0.8rem;text-align:center;white-space:nowrap">${lastMail}</td>
        <td style="padding:10px 8px;text-align:center">${opened}</td>
        <td style="padding:10px 8px;text-align:center">
          <button onclick="sendRetentionNow(${u.id}, this)"
            style="background:#a78bfa18;border:1px solid #a78bfa44;border-radius:6px;padding:5px 12px;color:#a78bfa;cursor:pointer;font-size:0.78rem;white-space:nowrap">
            ${isEN ? 'Send' : isPT ? 'Enviar' : 'Enviar'}
          </button>
        </td>
      </tr>`;
  }).join('');
  ...
  el.innerHTML = `
    <div style="overflow-x:auto">
      <table style="width:100%;border-collapse:collapse;font-size:0.85rem">
        <thead>
          <tr style="color:var(--text3);font-size:0.72rem;text-transform:uppercase;letter-spacing:0.07em;border-bottom:1px solid var(--border)">
            <th style="padding:8px 12px;text-align:left;white-space:nowrap">${isEN ? 'Name' : isPT ? 'Nome' : 'Nombre'}</th>
            <th style="padding:8px 12px;text-align:left">Email</th>
            <th style="padding:8px;text-align:center">Mental</th>
            <th style="padding:8px;text-align:center">${isEN ? 'Tech' : 'Técnico'}</th>
            <th style="padding:8px;text-align:center">${isEN ? 'AI' : 'Perfil IA'}</th>
            <th style="padding:8px;text-align:center">${isEN ? 'Hand' : isPT ? 'Mão' : 'Mano'}</th>
            <th style="padding:8px;text-align:center;white-space:nowrap">${isEN ? 'Last email' : isPT ? 'Último' : 'Último mail'}</th>
...
async function sendRetentionAll(btn) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      btn.style.color = '#4ade80';
      btn.style.borderColor = '#4ade8055';
      if (res.errors && res.errors.length) console.warn('[RETENTION] Errores:', res.errors);
      await loadRetentionData();
    } else {
