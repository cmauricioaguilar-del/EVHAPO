...
const GUIDE_STEPS = [
  {
    ...
    descEN: 'Diagnose your psychological leaks: tilt, discipline, emotional management and concentration. ~20 min.',
    color: '#818cf8',
  ...
  },
  {
    ...
    descEN: "Assess your Hold'em mastery: pre-flop, post-flop, position, ranges and spot analysis. ~20 min.",
    color: '#d4af37',
  ...
  },
  {
    ...
    descEN: 'Generate your comprehensive analysis correlating both tests. The AI identifies your player archetype and key gaps.',
    color: '#4DB6AC',
    action: null,
    noteES: 'Disponible después de completar ambos tests',
    notePT: 'Disponível após completar ambos os testes',
    noteEN: 'Available after completing both tests',
  },
  {
    num: 4,
    icon: '📚',
    ...
    descEN: 'Get a personalised 4-week plan with goals, exercises and resources tailored to your gaps.',
    color: '#818cf8',
  ...
  },
  {
    ...
    titleEN: 'Hand Analysis',
    descES: 'Sube historiales de torneos (archivos .txt o .zip) para que la IA analice tus spots y detecte patrones de error.',
    descPT: 'Envie históricos de torneios (arquivos .txt ou .zip) para que a IA analise seus spots e detecte padrões de erro.',
    descEN: 'Upload tournament hand histories (.txt or .zip files) so the AI can analyse your spots and detect error patterns.',
    color: '#f472b6',
    action: null,
    noteES: 'Desde el Dashboard → pestaña Torneo',
    notePT: 'No Dashboard → aba Torneio',
    noteEN: 'From the Dashboard → Hand Analysis tab',
  },
  {
    num: 6,
    icon: '📊',
    ...
    descEN: 'Log your sessions, analyse your results curve and verify if your bankroll supports your current stakes.',
    color: '#4ade80',
  ...
  },
  {
    num: 7,
    icon: '📒',
    titleES: 'Exporta tu Cuaderno de Control',
    titlePT: 'Exporte seu Caderno de Controle',
    titleEN: 'Export your Control Workbook',
    descES: 'Descarga tu cuaderno personalizado en PDF o Excel para llevar tu propio registro del avance semana a semana. Incluye tu diagnóstico, gráficos de radar y las 12 semanas de plan con seguimiento.',
    descPT: 'Baixe seu caderno personalizado em PDF ou Excel para manter seu próprio registro do progresso semana a semana. Inclui diagnóstico, gráficos de radar e as 12 semanas de plano com acompanhamento.',
    descEN: 'Download your personalised workbook in PDF or Excel to keep your own record of week-by-week progress. Includes your diagnosis, radar charts and 12-week plan with tracking.',
    color: '#d4af37',
    action: "generateWorkbook()",
    btnES: 'Exportar PDF →',
    btnPT: 'Exportar PDF →',
    btnEN: 'Export PDF →',
  ...
  },
  {
    ...
    descEN: 'Repeat both tests after working the plan. The Evolution tab will show your real progress.',
    color: '#fb923c',
    action: null,
    noteES: 'El tab Evolución aparece automáticamente',
    notePT: 'A aba Evolução aparece automaticamente',
    noteEN: 'The Evolution tab appears automatically',
  },
];
...
function showGuide() {
  ...
  const stepsHtml = GUIDE_STEPS.map((s, i) => {
    ...
    const title  = isEN ? s.titleEN : isPT ? s.titlePT : s.titleES;
    const desc   = isEN ? s.descEN  : isPT ? s.descPT  : s.descES;
    const note   = isEN ? s.noteEN  : isPT ? s.notePT  : s.noteES;
    const btn    = isEN ? s.btnEN   : isPT ? s.btnPT   : s.btnES;
    ...
    <div id="guide-card" style="background:var(--card);border:1px solid var(--border);border-radius:16px;width:100%;max-width:520px;max-height:92vh;overflow-y:auto;box-shadow:0 24px 80px rgba(0,0,0,0.6)">
      ...
      <div style="position:sticky;top:0;z-index:2;background:var(--card);border-bottom:1px solid var(--border);padding:18px 22px 14px;border-radius:16px 16px 0 0">
        <div style="display:flex;align-items:center;justify-content:space-between">
          <div>
            <h2 style="margin:0;font-size:1.1rem;font-weight:800;color:var(--accent)">${isEN ? '📖 How to use MindEV-IA' : isPT ? '📖 Como usar o MindEV-IA' : '📖 Cómo usar MindEV-IA'}</h2>
            <p style="margin:4px 0 0;font-size:0.78rem;color:var(--text3)">${isEN ? 'Recommended flow for maximum improvement' : isPT ? 'Fluxo recomendado para máxima melhoria' : 'Flujo recomendado para máxima mejora'}</p>
          </div>
          <div style="display:flex;gap:8px;align-items:center">
            <button onclick="downloadGuidePDF()" title="${isEN ? 'Download PDF' : isPT ? 'Baixar PDF' : 'Descargar PDF'}"
...
function downloadGuidePDF() {
  ...
  if (typeof window.jspdf === 'undefined' && typeof window.jsPDF === 'undefined') {
    alert(isEN ? 'PDF library not available.' : isPT ? 'Biblioteca PDF não disponível.' : 'Librería PDF no disponible.');
    return;
  }
  ...
  doc.text(isEN ? 'User guide — Recommended flow' : isPT ? 'Guia de uso — Fluxo recomendado' : 'Instructivo de uso — Flujo recomendado', margin, 22);
  doc.setFontSize(8);
  ...
  const footerTxt = isEN
    ? "MindEV-IA © 2025 — Diagnostic platform for Texas Hold'em players"
    : isPT
  ...
  doc.save(isEN ? 'mindev-ia-guide.pdf' : isPT ? 'guia-mindev-ia.pdf' : 'instructivo-mindev-ia.pdf');
}
