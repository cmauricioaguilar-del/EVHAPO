...
const EVHAPO_CATEGORIES = [
  ...
  },
  {
    ...
    tips: [
      ...
      'Crea una señal de alarma personal (una palabra o gesto) que actives cuando notes los primeros síntomas de tilt.',
      'Estudia los diferentes tipos de tilt: por mala suerte, por mal juego propio, por jugadores irritantes.',
      'Implementa un descanso obligatorio de 10 minutos cada vez que pierdas 3 manos seguidas.',
      'Practica la técnica del "observador externo": imagina que analizas tu juego como si fueras otro jugador.',
    ],
  },
  {
    ...
    tips: [
      ...
      'Busca un grupo de estudio o "study group" con jugadores de tu nivel para crecer juntos.',
      'Usa software de análisis (GTO Wizard, Pio Solver) para entender las brechas entre tu juego y el óptimo.',
      'Establece metas de aprendizaje mensuales específicas: dominar un concepto de rangos, mejorar el c-bet en flops secos, etc.',
    ],
  },
];
...
function getLevel(pct) {
  ...
  if (pct >= 85) return { label: isEN ? 'ELITE'           : isPT ? 'ÉLITE'             : 'ÉLITE',          cls: 'level-elite',      desc: isEN ? 'Elite player mindset. You are operating at the highest level!'                          : isPT ? 'Mentalidade de jogador de alto desempenho. Você está no mais alto nível!'          : 'Mentalidad de jugador de alto rendimiento. ¡Estás al más alto nivel!' };
  if (pct >= 70) return { label: isEN ? 'ADVANCED'        : isPT ? 'AVANÇADO'          : 'AVANZADO',       cls: 'level-avanzado',   desc: isEN ? 'Solid strengths with small areas to refine before reaching elite level.'                 : isPT ? 'Forças sólidas com pequenas áreas de melhoria para chegar à elite.'               : 'Fortalezas sólidas con pequeñas áreas de mejora para llegar a la élite.' };
  if (pct >= 50) return { label: isEN ? 'INTERMEDIATE'    : isPT ? 'INTERMEDIÁRIO'     : 'INTERMEDIO',     cls: 'level-intermedio', desc: isEN ? 'Solid foundation with significant opportunities for improvement.'                        : isPT ? 'Base sólida com oportunidades de melhoria significativas.'                        : 'Base sólida con oportunidades de mejora significativas.' };
  return           { label: isEN ? 'DEVELOPING'       : isPT ? 'EM DESENVOLVIMENTO' : 'EN DESARROLLO',  cls: 'level-basico',     desc: isEN ? 'Great growth potential ahead. This diagnosis is your first step!'                      : isPT ? 'Grande potencial de crescimento. Este diagnóstico é o primeiro passo!'           : 'Gran potencial de crecimiento. ¡Este diagnóstico es el primer paso!' };
}
